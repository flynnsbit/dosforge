...
def test_windows_vhd_pipeline_accepts_freedos_fat32(tmp_path: Path):
    ...
    It may fail later at the boot resolver step if no assets are present,
    but the pipeline gate itself must let it through.
    """
    ...
    try:
        ...
        assert "not yet supported on this platform" not in str(exc), (
            f"FreeDOS FAT32 should not be blocked by the unsupported-mode gate, got: {exc}"
        )
        assert "currently restricted to FAT16" not in str(exc), (
            f"FreeDOS FAT32 restriction should be lifted, got: {exc}"
        )


def test_windows_vhd_pipeline_accepts_freedos_and_msdos71(tmp_path: Path):
    """FreeDOS (FAT16 and FAT32) and MS-DOS 7.1 don't raise the unsupported-mode ValidationError.
